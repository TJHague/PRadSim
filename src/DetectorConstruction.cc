...
{
    if (fConfig != "prad" && fConfig != "drad" && fConfig != "test" && fConfig != "X17")
        fConfig = "prad";
    ...
    if (fConfig == "drad") {
        ...
        // Variables that are different for X17 setup
        fWorldSizeZ = 800 * cm;
        fTargetCenter = -455.0 * cm;
        fTargetHalfL = 0.5 * um;
        fTargetMat = "Ta";
    } else {
        fTargetSDOn = false;
...
G4VPhysicalVolume *DetectorConstruction::DefineX17Volumes()
{
...
void DetectorConstruction::DefineX17SDs()
{
    if (fTargetSDOn) {
